package com.kluksa.financije.tmgr.web.rest;
...
import com.kluksa.financije.tmgr.config.KafkaProperties;
...
class TransactionManagerKafkaResourceIT {
    ...
    void setup() {
        ...
        TransactionManagerKafkaResource kafkaResource = new TransactionManagerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/transaction-manager-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/transaction-manager-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
